...
### ChatModel의 스트림 응답을 JPA를 사용하여 영속화할 때 생기는 트랜젝션 세션 문제

#### 문제, 고민 그리고 해결

- 실시간으로 전달되는 채팅을 기록하기 위해서 JPA로 채팅에 대한 도메인을 작성하여 관리하려고 하였음
- 하지만 Spring AI 1.0의 ChatClient는 기본구현으로 ChatModel을 스트림 응답을 다루는 과정에서 리액티브 스레드로 전환하여 사용함.
- 리액티브 환경에서는 외부 스프링 빈 메소드를 호출할 때 @Transactional 등의 스프링 프록시를 기능을 사용할 수가 없음.
- 이렇게 되면 트랜젝션 세션이 없는 상태로 JPA가 사용되므로 연관 엔티티를 탐색시 Lazy Initialization 오류가 발생함
- 학습 결과에 따르면 @Transactional은 스레드의 ThreadLocal을 사용하는데 리액티브 환경에서는 I/O시 스레드 전환이 일어나기 때문으로 이해되었음.
- 이 문제를 극복하기 위한 방법으로 "R2DBC 를 사용하고 ReactorContext로 트랜젝션을 관리하는 방식"을 알게 되었으나 러닝커브가 크고 JPA와는 맞지 않다고 판단하였음.
- 따라서 Spring AI 1.0 ChatClient의 기본 구현인 리액티브 스레드와 JPA를 함께 사용하는 방법을 모색하게 되었음
- 고민 끝에 리액티브 환경에서 기록한 채팅을 바탕으로 스프링 이벤트로 발생시키고 이를 정상적인 트랜젝션 환경에서 이벤트를 수신하여 JPA로 영속화하기로 하였음
- 이 방식은 리액티브 환경에서 블로킹을 회피하면서도 저장 로직은 분리할 수 있음.